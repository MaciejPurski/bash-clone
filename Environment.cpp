...
void Environment::loadUserVariable() {
	...
	variablesMap_.insert(std::make_pair(std::string("USER"), std::unique_ptr<EnvironmentVariable>(new EnvironmentVariable("USER", std::string(userName), globalVariable))));
}

void Environment::loadHomeVariable() {
	const char *homeDir = getUserHomeDir();

	variablesMap_.insert(std::make_pair(std::string("HOME"), std::unique_ptr<EnvironmentVariable>(new EnvironmentVariable("HOME", std::string(homeDir), globalVariable))));
...
void Environment::loadPathVariable() {
	...
	variablesMap_.insert(std::make_pair(std::string("PATH"), std::unique_ptr<EnvironmentVariable>(new EnvironmentVariable("PATH", std::string(completePathValue), globalVariable))));
}
...
bool Environment::isGlobalVariable(const std::string &name) const {
	...
	if(it!=variablesMap_.end() && it->second->isGlobal())
		return true;
...
char **Environment::getEnvironment() {
	...
	int i=0;
	for(auto it = variablesMap_.begin(); it!=variablesMap_.end(); ++it)
	{
...
std::string Environment::searchPath(const std::string &name) {
	...
	for(auto it = pathPieces.begin(); it!=pathPieces.end(); ++it)
	{
		if(searchInDir(*it, name)){
			tmp += *it;
			break;
		}
	}
